                               ...
                               struct hmap *mcast_groups)
{
    ...
    /* Add ports to mrouters to the IGMP group. Traffic might need to be
     * routed by the mrouter.
     */
    for (size_t i = 0; i < igmp_group->datapath->n_router_ports; i++) {
        struct ovn_port *rtr_port =
            igmp_group->datapath->router_ports[i]->peer;

        if (rtr_port->od && rtr_port->od->mcast_info.rtr.relay) {
            ovn_multicast_add(mcast_groups, &igmp_group->mcgroup,
                              igmp_group->datapath->router_ports[i]);
        }
    }
}
                    ...
                    struct hmap *lflows)
{
    ...
    HMAP_FOR_EACH (od, key_node, datapaths) {
        ...
        /* L3 admission control: drop broadcast source, localhost source or
         * destination, and zero network source or destination
         * (priority 100). */
