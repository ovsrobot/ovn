...
bfd_table_check_default(const struct nbrec_bfd *nb_bt)
{
    ...
    if (!nb_bt->n_min_tx || !nb_bt->min_tx[0]) {
        int64_t min_tx = BFD_DEF_MINTX;
        nbrec_bfd_set_min_tx(nb_bt, &min_tx, 1);
    }

    if (!nb_bt->n_min_rx) {
        int64_t min_rx = BFD_DEF_MINRX;
        nbrec_bfd_set_min_rx(nb_bt, &min_rx, 1);
    }

    if (!nb_bt->detect_mult) {
        nbrec_bfd_set_detect_mult(nb_bt, BFD_DEF_DETECT_MULT);
    }
}
                         ...
                         const struct sbrec_bfd *sb_bt)
{
    ...
    if (nb_bt->min_tx[0] != sb_bt->min_tx) {
        sbrec_bfd_set_min_tx(sb_bt, nb_bt->min_tx[0]);
    }

    if (nb_bt->min_rx[0] != sb_bt->min_rx) {
