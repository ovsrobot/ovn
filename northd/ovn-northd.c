...
static void
build_lswitch_arp_nd_responder_op(struct ovn_port *op,
                                  struct hmap *lflows,
                                  struct hmap *ports,
                                  struct ds *match,
                                  struct ds *actions)
 ...
 * (priority 0)*/
static void
build_lswitch_arp_nd_responder_od(struct ovn_datapath *od,
                                  struct hmap *lflows)
{
    if (od->nbs) {
 ...
 * (priority 110)*/
static void
build_lswitch_arp_nd_responder_lb(struct ovn_northd_lb *lb,
                                  struct hmap *lflows,
                                  struct ds *match,
                                  struct ds *actions)
{
 ...
 */
static void
build_lswitch_dns_lookup_response(struct ovn_datapath *od,
                                  struct hmap *lflows)
{
...
static void
build_lswitch_dns_and_dhcp_defaults(struct ovn_datapath *od,
                                  struct hmap *lflows)
{
    if (od->nbs) {
...
static void
build_lswitch_destination_bmcast_handling(struct ovn_datapath *od,
                                  struct hmap *lflows,
                                  struct ds *actions)
{
    if (!od->nbs) {
...
*/
static void
build_lrouter_arp_nd_flows(struct ovn_datapath *od, struct hmap *lflows)
{
    if (!od->nbr) {
        return;
    }
...
static void
build_lrouter_ip_input_v4(struct ovn_port *op, struct hmap *lflows,
                           struct ds *match, struct ds *actions)
{
    if (!op->nbrp) {
        return;
                                        ...
                                        struct lswitch_flow_build_info *lsi)
{
    ...
    build_lswitch_dns_lookup_response(od, lsi->lflows);
    build_lswitch_dns_and_dhcp_defaults(od, lsi->lflows);
    build_lswitch_destination_bmcast_handling(od, lsi->lflows,
                                              &lsi->actions);
    ...
    build_lrouter_arp_nd_flows(od, lsi->lflows);
                                        ...
                                        struct lswitch_flow_build_info *lsi)
{
    ...
    build_ipv6_input_flows_for_lrouter_port(op, lsi->lflows,
                                            &lsi->match, &lsi->actions);
    build_lrouter_ip_input_v4(op, lsi->lflows, &lsi->match, &lsi->actions);
}
