                              ...
                              struct lflow_ref *lflow_ref)
{
    ...
    if (!strcmp(acl->direction, "from-lport")) {
        ...
        ovn_lflow_add_with_hint(lflows, od, S_SWITCH_IN_ACL_EVAL, 65532,
                                acl->match, action, &acl->header_, lflow_ref);
    } else {
        ovn_lflow_add_with_hint(lflows, od, S_SWITCH_OUT_PRE_ACL, 65532,
                                acl->match, "next;", &acl->header_, lflow_ref);
        ovn_lflow_add_with_hint(lflows, od, S_SWITCH_OUT_ACL_EVAL, 65532,
                                acl->match, action, &acl->header_, lflow_ref);
    }
}
