                            ...
                            const struct chassis_features *features)
{
    ...
    if (features->ct_commit_to_zone && features->ct_next_zone &&
        ...
        !smap_get_bool(&od->nbr->options, "commit-all", false)) {
...
build_gw_lrouter_commit_all(const struct ovn_datapath *od,
                            struct lflow_table *lflows,
                            bool stateful_snat, bool stateful_dnat,
                            const struct chassis_features *features,
                            struct lflow_ref *lflow_ref)
{
    ...
    struct lflow_ref *lflow_ref)
{
    ...
    bool commit_all = smap_get_bool(&od->nbr->options, "commit-all", false);
    ...
    if (commit_all) {
        if (od->is_gw_router) {
            build_gw_lrouter_commit_all(od, lflows, stateful_snat,
                                        stateful_dnat, features, lflow_ref);
        }

        for (size_t i = 0; i < od->n_l3dgw_ports; i++) {
            struct ovn_port *l3dgw_port = od->l3dgw_ports[i];
            build_dgp_lrouter_commit_all(od, l3dgw_port, lflows,
                                         stateful_snat, stateful_dnat,
