...
static const struct ipv4_netaddr *lrp_get_primary_ipv4_addr(
    const struct ovn_port *op);
                               ...
                               struct ds *actions)
{
    ...
    const struct ipv4_netaddr *lrp_ipv4_addr = lrp_get_primary_ipv4_addr(rp);
    if (!lrp_ipv4_addr) {
        return;
    }
...
lrp_get_primary_ipv4_addr(const struct ovn_port *op)
{
    if (!op->lrp_networks.n_ipv4_addrs) {
        return NULL;
    }
                                        ...
                                        const struct shash *meter_groups)
{
    ...
    /* Use the LRP's primary IPv4 addr for the DHCP relay. */
    const struct ipv4_netaddr *lrp_ipv4_addr = lrp_get_primary_ipv4_addr(op);
    if (!lrp_ipv4_addr) {
        return;
    }
