                               ...
                               bool ct_lb_mark)
{
    ...
    struct ovn_datapath **lb_force_snat_router =
        xcalloc(lb->n_nb_lr, sizeof *lb_force_snat_router);
    int n_lb_force_snat_router = 0;

    struct ovn_datapath **lb_affinity_router =
        xcalloc(lb->n_nb_lr, sizeof *lb_affinity_router);
    int n_lb_affinity_router = 0;
    ...
    for (size_t i = 0; i < lb->n_nb_lr; i++) {
        ...
        if (!lport_addresses_is_empty(&od->lb_force_snat_addrs) ||
            od->lb_force_snat_router_ip) {
            lb_force_snat_router[n_lb_force_snat_router++] = od;
        } else {
            lb_affinity_router[n_lb_affinity_router++] = od;
        }
    ...
    build_lb_affinity_flows(lflows, lb, lb_vip, new_match,
                            ...
                            lb_force_snat_router, n_lb_force_snat_router,
                            true);
    ...
    build_lb_affinity_flows(lflows, lb, lb_vip, new_match, lb_aff_action,
                            lb_affinity_router, n_lb_affinity_router, true);
    ...
    free(lb_force_snat_router);
    free(lb_affinity_router);
    free(gw_router);
}
