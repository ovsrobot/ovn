...
#define REGBIT_LB_L2_AFF_BACKEND_IP6 "xxreg0"

/* Register definitions for switches and routers. */

/* Register used to store backend ipv4 address
 * for load balancer affinity. */
#define REGBIT_LB_AFF_BACKEND_IP4 "reg4"
...
/* Register used to store backend ipv6 address
 * for load balancer affinity. */
#define REGBIT_LB_L3_AFF_BACKEND_IP6 "xxreg1"
 ...
 * |    |     REGBIT_ACL_LABEL                         | X |                                   |
 * +----+----------------------------------------------+ X |                                   |
 * | R5 |                   UNUSED                     | X |   REGBIT_LB_L2_AFF_BACKEND_IP6    |
 ...
 * |     |                           | 1 |                 |   |                                    |
 * +-----+---------------------------+---+-----------------+---+------------------------------------+
 * | R4  | REGBIT_LB_AFF_BACKEND_IP4 | X |                 |   |                                    |
 * |     |                           | R |                 |   |                                    |
 * +-----+---------------------------+ E |     UNUSED      | X |                                    |
 * | R5  |        UNUSED             | G |                 | X |                                    |
 * |     |                           | 2 |                 | R |     REGBIT_LB_L3_AFF_BACKEND_IP6   |
 * +-----+---------------------------+---+-----------------+ E |                                    |
 * | R6  |        UNUSED             | X |                 | G |                                    |
 * |     |                           | R |                 | 1 |                                    |
 * +-----+---------------------------+ E |     UNUSED      |   |                                    |
                        ...
                        bool router_pipeline)
{
    ...
    for (size_t i = 0; i < lb_vip->n_backends; i++) {
        ...
        if (backend->port) {
            ds_put_format(&aff_match_learn, " && %s && %s == %d",
                          lb->proto,
                          router_pipeline
                            ? REG_ORIG_TP_DPORT_ROUTER : REG_ORIG_TP_DPORT,
                          backend->port);
        }
        ...
        if (backend->port) {
            ds_put_format(&aff_match,
                ...
                IN6_IS_ADDR_V4MAPPED(&lb_vip->vip)
                    ? REGBIT_LB_AFF_BACKEND_IP4
                    : router_pipeline
                        ? REGBIT_LB_L3_AFF_BACKEND_IP6
                        : REGBIT_LB_L2_AFF_BACKEND_IP6,
                backend->ip_str, backend->port);
            ds_put_format(&aff_action_lb, "ct_lb_mark(backends=%s%s%s:%d);",
                          ipv6 ? "[" : "", backend->ip_str,
                          ipv6 ? "]" : "", backend->port);
        } else {
            ds_put_format(&aff_match,
        ...
        for (size_t j = 0; j < n_dp; j++) {
            ...
            if (!ovn_dp_group_add_with_reference(lflow_ref_aff_learn, od)) {
                lflow_ref_aff_learn = ovn_lflow_add_at_with_hash(
                        lflows, od, stage0, 100,  ds_cstr(&aff_match_learn),
