                           ...
                           const struct ovn_datapaths *lr_datapaths)
{
    ...
    ovn_lflow_add_with_dp_group(
        ...
        100, new_lb_match, aff_check, &lb->nlb->header_);
                                  ...
                                  const unsigned long *dp_bitmap)
{
    ...
    ovn_lflow_add_with_dp_group(
        ctx->lflows, dp_bitmap, ods_size(lr_datapaths), S_ROUTER_IN_DNAT,
        ctx->prio, ds_cstr(ctx->est_match), ctx->est_action[type],
        &ctx->lb->nlb->header_);
}
                                  ...
                                  struct ds *match)
{
    ...
    for (size_t i = 0; i < lb->n_vips; i++) {
        ...
        ovn_lflow_add_with_dp_group(
            ...
            prio, ds_cstr(match), ds_cstr(&defrag_actions), &lb->nlb->header_);
    }
