                  ...
                  const struct smap *health_check_options)
{
    ...
    bool port_up = false;
    enum nf_port_binding_state port_state = network_function_port_binding_state(
                                           ports, n_ports, ls_ports,
                                           &chassis_name);
                         ...
                         struct lflow_ref *lflow_ref)
{
    ...
    const char *egress_action = (has_stateful_acl && !lsp_is_localnet(op->nbsp))
                                ? "next;"
...
static const char*
network_function_group_get_fallback(const struct nbrec_network_function_group *nfg)
{
    if (nfg->fallback) {
        return nfg->fallback;
    }
                               ...
                               bool ingress, struct lflow_ref *lflow_ref)
{
    ...
    */
     ...
     * of the ingress pipeline. Original packet continues with ingress pipeline.
     *
    ...
    ds_put_format(&match, REGBIT_NF_ENABLED" == 1 && "
                 REGBIT_NF_ORIG_DIR" == 0 && "
                 "ct_label.nf_group_id == %"PRIu8, (uint8_t) nfg->id);
    ovn_lflow_add(lflows, od, rev_stage, 99, ds_cstr(&match), ds_cstr(&action),
                  lflow_ref);
    ds_clear(&match);
    ds_clear(&action);
