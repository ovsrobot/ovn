    ...
    struct hmap *svc_monitor_map)
{
    ...
    build_lswitch_lbs_from_lrouter(lr_datapaths, ls_datapaths,
                                   lb_dps_map, lb_group_dps_map);
}
                         ...
                         struct northd_data *nd)
{
    ...
    NBREC_LOGICAL_SWITCH_TABLE_FOR_EACH_TRACKED (
         ...
         * then we can't handle handle this yet.  Return false. */
    ...
    const struct ovn_datapaths *lr_datapaths)
{
    ...
    SBREC_DATAPATH_BINDING_TABLE_FOR_EACH_TRACKED (sbrec_dp, sbrec_dp_table) {
        ...
        if (sbrec_datapath_binding_is_new(sbrec_dp)) {
            if (!od) {
                VLOG_WARN_RL(&rl, "A datapath-binding for %s is created but "
                             "the %s is not found.", name,
            ...
            if (od && od->sb == sbrec_dp) {
                VLOG_WARN_RL(&rl, "A datapath-binding for %s is deleted but "
                             "the %s still exists.", name,
                            is_switch ? "Logical switch" : "Logical router");
                return false;
            }
        } else {
            /* sbrec_dp is updated.*/
            if (!od) {
                VLOG_WARN_RL(&rl, "A datapath-binding for %s is updated but "
                             "the %s is not found.", name,
                             is_switch ? "Logical switch" : "Logical router");
                return false;
            }
            if (od->sb != sbrec_dp) {
                VLOG_WARN_RL(&rl, "A datapath-binding for %s is updated with "
                             "a new IDL row, which is unusual.", name);
                return false;
            }
        }
    }
             ...
             struct ovsdb_idl_txn *ovnsb_txn)
{
    ...
    build_lb_port_related_data(ovnsb_txn,
                               ...
                               &data->lr_datapaths,
                               &data->ls_datapaths, &data->ls_ports,
