               ...
               struct hmap *sr_bfd_connections)
{
    ...
    struct hmap bfd_map = HMAP_INITIALIZER(&bfd_map);

    struct bfd_entry *bfd_e;
    HMAP_FOR_EACH (bfd_e, hmap_node, bfd_connections) {
        /* Make a private copy of bfd_connections map in orther to allow
         * writing into map entries. */
        struct bfd_entry *e = bfd_alloc_entry(&bfd_map, bfd_e->logical_port,
