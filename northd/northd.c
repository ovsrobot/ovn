                    ...
                    const struct ovn_datapath_binding *dp)
{
    ...
    od->tunnel_key = dp->sb_dp->tunnel_key;
                                     ...
                                     struct ovn_datapath *od)
{
    ...
    sbrec_ip_multicast_set_datapath(sb, od->dp->sb_dp);
...
{
    char *mp_name = ovn_mirror_port_name(ovn_datapath_name(op->od->dp->sb_dp),
                                         nb_mirror->sink);
                       ...
                       struct hmapx *mirror_attached_ports)
{
    ...
    if (op && (op->od || op->nbsp || op->nbrp)) {
        ...
        return NULL;
    } else if (op && (!op->sb || op->sb->datapath == od->dp->sb_dp)) {
        /*
               ...
               struct ovs_list *both_dbs, struct ovs_list *nb_only)
{
    ...
    if (crp && crp->sb && crp->sb->datapath == op->od->dp->sb_dp) {
...
{
    sbrec_port_binding_set_datapath(op->sb, op->od->dp->sb_dp);
    if (op->nbrp) {
    ...
    struct hmap *ls_ports, struct hmap *lr_ports)
{
    ...
    LIST_FOR_EACH_SAFE (op, list, &both) {
         ...
         */
        if (op->od->dp->sb_dp != op->sb->datapath) {
                    ...
                    struct lflow_table *lflows)
{
    ...
    for (size_t i = 0; i < op->nbsp->n_mirror_rules; i++) {
        ...
        char *serving_port_name = ovn_mirror_port_name(
                                        ovn_datapath_name(op->od->dp->sb_dp),
                                        mirror->sink);
                 ...
                 struct hmap *ls_datapaths)
{
    ...
    HMAP_FOR_EACH (od, key_node, ls_datapaths) {
        ...
        for (size_t i = 0; i < od->nbs->n_dns_records; i++) {
            ...
            dns_info->sbs[dns_info->n_sbs - 1] = od->dp->sb_dp;
        }
    }
               ...
               struct hmap *ls_datapaths)
{
    ...
    HMAP_FOR_EACH (od, key_node, ls_datapaths) {
        ...
        const struct sbrec_ip_multicast *ip_mcast =
            ip_mcast_lookup(sbrec_ip_mcast_by_dp, od->dp->sb_dp);
    ...
    struct hmap *lr_ports)
{
    ...
    SBREC_STATIC_MAC_BINDING_TABLE_FOR_EACH_SAFE (sb_smb,
        ...
        if (!op || !op->nbrp || !op->od || !op->od->dp->sb_dp) {
            sbrec_static_mac_binding_delete(sb_smb);
        }
    }
        ...
        if (op && op->nbrp) {
            ...
            if (od && od->dp->sb_dp) {
                ...
                if (!mb) {
                    ...
                    sbrec_static_mac_binding_set_datapath(mb, od->dp->sb_dp);
                } else {
