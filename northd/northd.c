...
build_lb_affinity_flows(struct hmap *lflows, struct ovn_northd_lb *lb,
                        struct ovn_lb_vip *lb_vip OVS_UNUSED,
                        struct ovn_northd_lb_vip *lb_vip_nb, char *match,
                        bool router_pipeline)
{
    if (lb_vip_nb->lb_health_check || lb->affinity_timeout <= 0) {
        return;
    }
    ...
    for (size_t i = 0; i < lb_vip->n_backends; i++) {
        ...
        ds_put_format(&aff_action_learn,
                      "commit_lb_aff(vip = \"%s:%d\", backend = \"%s:%d\", "
                      "proto = %s, timeout = %d); next;",
                      lb_vip->vip_str, lb_vip->vip_port,
                      backend->ip_str, backend->port, lb->proto,
                      lb->affinity_timeout);
               ...
               const struct shash *meter_groups)
{
    for (size_t i = 0; i < lb->n_vips; i++) {
        ...
        if (!reject) {
            build_lb_affinity_flows(lflows, lb, lb_vip, lb_vip_nb,
                                    ds_cstr(match), false);
        }
                               ...
                               bool ct_lb_mark)
{
    ...
    if (!reject) {
        build_lb_affinity_flows(lflows, lb, lb_vip, vips_nb, new_match, true);
    }
