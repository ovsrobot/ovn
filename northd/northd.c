...
join_datapaths(const struct nbrec_logical_switch_table *nbrec_ls_table,
               ...
               struct ovs_list *lr_list, bool *ic_mode)
{
...
uint32_t
get_ovn_max_dp_key_local(bool _vxlan_mode, bool ic_mode)
{
    if (_vxlan_mode) {
        /* OVN_MAX_DP_GLOBAL_NUM doesn't apply for VXLAN mode. */
        return ic_mode ? OVN_MAX_DP_VXLAN_KEY_LOCAL : OVN_MAX_DP_VXLAN_KEY;
    }
    return OVN_MAX_DP_KEY_LOCAL;
}
                          ...
                          bool ic_mode)
{
    if (!od->tunnel_key) {
        od->tunnel_key = ovn_allocate_tnlid(dp_tnlids, "datapath",
            ...
            get_ovn_max_dp_key_local(vxlan_mode, ic_mode), hint);
        if (!od->tunnel_key) {
            if (od->sb) {
                ...
                struct ovs_list *lr_list)
{
    ...
    join_datapaths(nbrec_ls_table, nbrec_lr_table, sbrec_dp_table, ovnsb_txn,
                   datapaths, &sb_only, &nb_only, &both, lr_list, &ic_mode);
    ...
    LIST_FOR_EACH_SAFE (od, list, &both) {
        ovn_datapath_allocate_key(datapaths, &dp_tnlids, od, &hint, ic_mode);
    }
