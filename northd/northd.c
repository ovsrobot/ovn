...
bfd_table_sync(struct ovsdb_idl_txn *ovnsb_txn,
               ...
               struct hmap *rp_bfd_connections,
               struct hmap *sr_bfd_connections)
{
    if (!ovnsb_txn) {
        return;
    }
    ...
    NBREC_BFD_TABLE_FOR_EACH (nb_bt, nbrec_bfd_table) {
        ...
        struct ovn_port *op = bfd_e->op;
        if (!op) {
            continue;
        }

        if (!bfd_port_lookup(rp_bfd_connections, nb_bt->logical_port,
                             nb_bt->dst_ip) &&
            !bfd_port_lookup(sr_bfd_connections, nb_bt->logical_port,
                             nb_bt->dst_ip)) {
            /* no consumer for this bfd connection, mark it as admin_down. */
            bfd_set_status(bfd_e, "admin_down");
        }
        nbrec_bfd_set_status(nb_bt, bfd_e->status);
        ...
        if (!bfd_e->sb_bt) {
            ...
            if (strcmp(bfd_e->sb_bt->status, nb_bt->status)) {
                if (!strcmp(nb_bt->status, "admin_down") ||
                    ...
                    bfd_set_status(bfd_e, bfd_e->sb_bt->status);
                }
            }
    ...
    for (size_t i = 0; i < rule->n_bfd_sessions; i++) {
        ...
        if (!bfd_port_lookup(bfd_active_connections, nb_bt->logical_port,
            ...
            bfd_alloc_entry(bfd_active_connections, nb_bt->logical_port,
                            nb_bt->dst_ip, bfd_e->status);
        }

        return strcmp(bfd_e->status, "down");
    }
...
build_static_route_flows_for_lrouter(
        ...
        struct simap *route_tables, struct lflow_ref *lflow_ref)
{
    ovs_assert(od->nbr);
