                    ...
                    enum route_source source)
{
    ...
    uint32_t hash = uuid_hash(&od->dp->sb_dp->header_.uuid);
              ...
              const struct sbrec_port_binding *tracked_port)
{
    ...
    HMAP_FOR_EACH_WITH_HASH (route_e, hmap_node, hash, route_map) {
        if (!uuid_equals(&sb_db->header_.uuid,
                         &route_e->od->dp->sb_dp->header_.uuid)) {
            continue;
        }
                         ...
                         struct hmap *routes)
{
    ...
    for (size_t i = 0; i < lr_nat->n_nat_entries; i++) {
        ...
        if (!ar_entry_find(routes, advertising_od->dp->sb_dp, advertising_op->sb,
                           nat->nb->external_ip,
    ...
    struct advertised_route_sync_data *data)
{
    ...
    HMAP_FOR_EACH (route_e, hmap_node, dynamic_routes) {
        ...
        if (ar_entry_find(&sync_routes, route_e->od->dp->sb_dp, route_e->op->sb,
    ...
    HMAP_FOR_EACH_POP (route_e, hmap_node, &sync_routes) {
        ...
        sbrec_advertised_route_set_datapath(sr, route_e->od->dp->sb_dp);
