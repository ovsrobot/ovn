                       ...
                       struct engine_arg *arg OVS_UNUSED)
{
    ...
    *data = (struct dynamic_routes_data) {
        ...
        .trk_data.trk_matched_parsed_routes =
            HMAPX_INITIALIZER(&data->trk_data.trk_matched_parsed_routes),
    };
...
dynamic_routes_clear_tracked(struct dynamic_routes_data *data)
{
    ...
    HMAPX_FOR_EACH_SAFE (hmapx_node,
                         &data->trk_data.trk_matched_parsed_routes) {
        parsed_route_free(hmapx_node->data);
        hmapx_delete(&data->trk_data.trk_matched_parsed_routes, hmapx_node);
    }
    data->tracked = false;
}
...
en_dynamic_routes_cleanup(void *data_)
{
    ...
    hmapx_destroy(&data->trk_data.trk_matched_parsed_routes);
                           ...
                           struct hmap *old_parsed_routes)
{
    ...
    HMAP_FOR_EACH (pr, key_node, &data->parsed_routes) {
    ...
    HMAP_FOR_EACH_SAFE (pr, key_node, old_parsed_routes) {
        if (pr->stale) {
            ...
            hmapx_add(&data->trk_data.trk_matched_parsed_routes, pr);
        }
    }
