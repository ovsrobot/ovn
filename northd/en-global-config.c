...
{
    char *old_ip4_p = CONST_CAST(char *, *old_ip4_pptr);
    ...
    if (new_ip4) {
        ...
        if (inet_parse_address(new_ip4, &svc_mon_addr)
            ...
            if ((old_ip4_p == NULL)
                || strcmp(old_ip4_p, new_ip4)) {
                if (old_ip4_p) {
                    free(old_ip4_p);
                }
                *old_ip4_pptr = xstrdup(new_ip4);
            }
        } else {
            if (old_ip4_p) {
                free(old_ip4_p);
...
void en_global_config_cleanup(void *data OVS_UNUSED)
{
    ...
    if (config_data->svc_monitor_ip4) {
        free(CONST_CAST(char *, config_data->svc_monitor_ip4));
    }
    if (config_data->svc_monitor_ip4_dst) {
        free(CONST_CAST(char *, config_data->svc_monitor_ip4_dst));
    }
