...
find_veth_peer(const char *ifname)
{
    ...
    if (sock < 0) {
        ...
        VLOG_DBG_RL(&rl, "veth peer auto-discovery: Failed to create socket: %s",
            ovs_strerror(errno));
        return NULL;
    }
                ...
                if (redirect_port) {
                    ...
                    if (ifname) {
                        ...
                        if (peer_iface) {
                            ...
                            static struct vlog_rate_limit rl =
                                VLOG_RATE_LIMIT_INIT(5, 20);
                            VLOG_INFO_RL(&rl, "Cannot auto-discover veth peer "
                                         "for port '%s' (bound to '%s'), falling "
                                         "back to learning routes from all ports",
