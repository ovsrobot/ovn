    ...
    SBREC_ADVERTISED_ROUTE_TABLE_FOR_EACH (route,
        ...
        if (route->tracked_port) {
            ...
            if (lport_is_local(r_ctx_in->sbrec_port_binding_by_name,
                ...
                /* If the route carries the full backend service selector, look up
                 * matching Service_Monitor rows on this chassis. Skip installing
                 * the route when matching rows exist but none report online.
                 * Routes without the full selector are installed unconditionally.
                 *
                 * The full selector (ip + port + protocol) is required to identify
                 * the row as LB-derived, since route_source is not stored on the
                 * SB row.  A partial selector could match an unrelated SM on the
                 * same (ip, port) for a different protocol or LB. */
