                         ...
                         struct ovsdb_idl_index *pb_by_dp)
{
    ...
    ct_zone_limits_sync_per_dp(ctx, dp, name, pb_by_dp);
                           ...
                           bool updated, int *scan_start)
{
    ...
    if (updated) {
        ...
        ct_zone_limit_sync(ctx, pb->logical_port, ct_zone_get_pb_limit(pb));
        return true;
...
void
ct_zones_sync_limit(struct ct_zone_ctx *ctx,
                    ...
                    struct ovsdb_idl_index *pb_by_dp)
{
    ...
    HMAP_FOR_EACH (ld, hmap_node, local_datapaths) {
        ...
        ct_zone_limits_sync_per_dp(ctx, ld->datapath, name, pb_by_dp);
    }
}
...
static void
ct_zone_limits_sync_per_dp(struct ct_zone_ctx *ctx,
                           const struct sbrec_datapath_binding *dp,
                           const char *name,
                           struct ovsdb_idl_index *pb_by_dp)
{
    if (smap_get(&dp->external_ids, "logical-switch")) {
        ...
        SBREC_PORT_BINDING_FOR_EACH_EQUAL (pb, target, pb_by_dp) {
            ct_zone_limit_sync(ctx, pb->logical_port,
    ...
    ct_zone_limit_sync(ctx, dnat, dp_limit);
    ct_zone_limit_sync(ctx, snat, dp_limit);
...
static void
ct_zone_limit_sync(struct ct_zone_ctx *ctx, const char *name, int64_t limit)
{
    struct ct_zone *ct_zone = shash_find_data(&ctx->current, name);
