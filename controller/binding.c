...
/* Check if a patch port needs CT zone allocation based on enable-router-port-acl.
 * Returns true if the patch port should be added to local_lports for CT zone
 * allocation.
 */
static bool
                    ...
                    const struct sbrec_port_binding *pb)
{
    ...
    case LP_PATCH:
        ...
        if (sbrec_port_binding_is_updated(pb, SBREC_PORT_BINDING_COL_OPTIONS)) {
            tracked_datapath_lport_add(pb, TRACKED_RESOURCE_UPDATED,
                b_ctx_out->tracked_dp_bindings);
            if (patch_port_needs_ct_zone(pb)) {
                update_local_lports(pb->logical_port, b_ctx_out, LPORT_STATUS_BOUND);
