                           ...
                           struct neighbor_ovn_maintain_entry *entry)
{
    ...
    *entry = (struct neighbor_ovn_maintain_entry) {
    ...
    if (nrm_mode_IP_is_set(redistribute_mode)) {
        ...
        /* The option may either name a logical router port or directly
         * specify the MAC address to be used on the bridge interface. The
         * latter is useful when the router port has multiple MAC addresses
         * and the CMS needs to pick a specific one. */
        struct eth_addr mac;
        if (eth_addr_from_string(lrp_name, &mac)) {
            entry->br_config.has_addr = true;
            entry->br_config.lladdr = mac;
            return true;
        }
