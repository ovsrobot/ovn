...
send_garp_rarp_wait(long long int send_garp_rarp_time)
{
     ...
     * be sent. */
    if (!cmap_is_empty(garp_rarp_get_data())) {
...
{
    const struct cmap *send_garp_rarp_data = garp_rarp_get_data();
    if (cmap_is_empty(send_garp_rarp_data)) {
        return;
    }
    ...
    *send_garp_rarp_time = LLONG_MAX;
    ...
    CMAP_FOR_EACH (garp, cmap_node, send_garp_rarp_data) {
    ...
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    if (garp_rarp_data_sync(reset_timers)) {
...
may_inject_pkts(void)
{
    return (!shash_is_empty(&ipv6_ras) ||
            !cmap_is_empty(garp_rarp_get_data()) ||
