               ...
               bool continuous)
{
    ...
    /* Update the announce_time and backoff. In case the announce_time has
     * been modified in the mean time by another thread we need to return the
     * new value as it will be used for wakeup. */
    if (!atomic_compare_exchange_strong(&garp_rarp->announce_time,
                                        &old_announce_time, announce_time)) {
        atomic_read(&garp_rarp->announce_time, &announce_time);
    }
