...
static void run_buffered_binding(const struct mac_bindings_map *recent_mbs)
...
static void
run_buffered_binding(const struct mac_bindings_map *recent_mbs)
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    SBREC_IGMP_GROUP_FOR_EACH_BYINDEX (sbrec_igmp, sbrec_igmp_groups) {
        ...
        if (!strcmp(sbrec_igmp->address, OVN_IGMP_GROUP_MROUTERS)) {
            continue;
        } else if (!ip_parse_mapped(sbrec_igmp->address, &group_addr)) {
            continue;
        }
