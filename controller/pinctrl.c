...
static long long int arp_max_timeout = GARP_RARP_DEF_MAX_TIMEOUT;
static bool arp_continuous;
...
static void destroy_send_arps(void);
static void send_garp_rarp_wait(long long int send_garp_rarp_time);
static void send_arp_wait(long long int send_arp_time);
...
static void send_garp_rarp_run(struct rconn *swconn,
    ...
    OVS_REQUIRES(pinctrl_mutex);
static void send_arp_run(struct rconn *swconn,
                         long long int *send_arp_time)
    OVS_REQUIRES(pinctrl_mutex);
...
pinctrl_handler(void *arg_)
{
    ...
    static long long int send_arp_time = LLONG_MAX;
    ...
    while (!latch_is_set(&pctrl->pinctrl_thread_exit)) {
        ...
        if (rconn_is_connected(swconn)) {
            ...
            if (may_inject_pkts()) {
                ...
                send_arp_run(swconn, &send_arp_time);
...
struct garp_rarp_data {
    ...
    ovs_be32 src_ipv4;           /* Ipv4 address of port. */
    ovs_be32 dst_ipv4;           /* Destination Ipv4 address. */
 ...
 * by pinctrl_mutex. */
static struct shash send_arp_data;

static void
init_send_arps(void)
{
    shash_init(&send_arp_data);
}

static void
destroy_send_arps(void)
{
    shash_destroy_free_data(&send_arp_data);
}

/* Runs with in the main ovn-controller thread context. */
static void
add_garp_rarp(const char *name, const struct eth_addr ea, ovs_be32 src_ip,
              ovs_be32 dst_ip, uint32_t dp_key, uint32_t port_key,
              struct shash *shash)
{
    struct garp_rarp_data *garp_rarp = xmalloc(sizeof *garp_rarp);
    garp_rarp->ea = ea;
    garp_rarp->src_ipv4 = src_ip;
    garp_rarp->dst_ipv4 = dst_ip;
                      ...
                      bool garp_continuous)
{
        ...
        while ((laddrs = shash_find_and_delete(nat_addresses,
            ...
            for (i = 0; i < laddrs->n_ipv4_addrs; i++) {
                ...
                if (garp_rarp) {
                    ...
                    add_garp_rarp(name, laddrs->ea, laddrs->ipv4_addrs[i].addr,
            ...
            if (laddrs->n_ipv4_addrs == 0) {
                    ...
                    if (garp_rarp) {
                        ...
                        add_garp_rarp(name, laddrs->ea, 0, 0,
                                      binding_rec->datapath->tunnel_key,
                                      binding_rec->tunnel_key,
                                      &send_garp_rarp_data);
                    }
    ...
    for (i = 0; i < binding_rec->n_mac; i++) {
        ...
        add_garp_rarp(binding_rec->logical_port, laddrs.ea, ip, ip,
                      binding_rec->datapath->tunnel_key,
...
static void
send_arp_update(const struct sbrec_port_binding *pb, const char *nexthop,
                long long int max_arp_timeout, bool continuous_arp)
{
    volatile struct garp_rarp_data *arp = shash_find_data(&send_arp_data,
                                                          nexthop);
    if (arp) {
        arp->dp_key = pb->datapath->tunnel_key;
        arp->port_key = pb->tunnel_key;
        if (max_arp_timeout != arp_max_timeout ||
            continuous_arp != arp_continuous) {
            /* reset backoff */
            arp->announce_time = time_msec() + 1000;
            arp->backoff = 1000; /* msec. */
        }
    } else {
        struct lport_addresses laddrs;
        ...
        if (laddrs.n_ipv4_addrs) {
            ...
            add_garp_rarp(nexthop, laddrs.ea, laddrs.ipv4_addrs[0].addr,
    ...
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    struct dp_packet packet;
    dp_packet_use_stub(&packet, packet_stub, sizeof packet_stub);
    if (garp_rarp->src_ipv4) {
        compose_arp(&packet, ARP_OP_REQUEST, garp_rarp->ea, eth_addr_zero,
                    true, garp_rarp->src_ipv4, garp_rarp->dst_ipv4);
    } else {
...
static void
send_arp_wait(long long int send_arp_time)
{
    /* Set the poll timer for next arp packet only if there is data to
     * be sent. */
    if (!shash_is_empty(&send_arp_data)) {
...
static long long int
send_arp(struct rconn *swconn, struct garp_rarp_data *garp_rarp,
         long long int current_time)
    OVS_REQUIRES(pinctrl_mutex)
{
    if (current_time < garp_rarp->announce_time) {
    ...
    /* Set the next announcement.  At most 5 announcements are sent for a
     * vif if arp_max_timeout is not specified otherwise cap the max
     * timeout to arp_max_timeout. */
    if (arp_continuous || garp_rarp->backoff < arp_max_timeout) {
        garp_rarp->announce_time = current_time + garp_rarp->backoff;
    } else {
        garp_rarp->announce_time = LLONG_MAX;
    }
    garp_rarp->backoff = MIN(arp_max_timeout, garp_rarp->backoff * 2);

    return garp_rarp->announce_time;
}

static void
send_arp_run(struct rconn *swconn, long long int *send_arp_time)
    OVS_REQUIRES(pinctrl_mutex)
{
    if (shash_is_empty(&send_arp_data)) {
        return;
    ...
    *send_arp_time = LLONG_MAX;
    ...
    struct shash_node *iter;
    SHASH_FOR_EACH (iter, &send_arp_data) {
        long long int next_announce = send_arp(swconn, iter->data,
                                               current_time);
        if (*send_arp_time > next_announce) {
            *send_arp_time = next_announce;
        }
    }
}
    ...
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    unsigned long long max_arp_timeout = GARP_RARP_DEF_MAX_TIMEOUT;
    bool garp_continuous = false, continuous_arp = true;
    ...
    if (cfg) {
        ...
        max_arp_timeout = smap_get_ullong(
                &cfg->external_ids, "arp-max-timeout-sec",
                GARP_RARP_DEF_MAX_TIMEOUT / 1000) * 1000;
        continuous_arp = !!max_arp_timeout;
    }
    ...
    SHASH_FOR_EACH_SAFE (iter, &send_arp_data) {
        if (!smap_get(&local_ecmp_nexthop_map, iter->name)) {
            send_garp_rarp_delete(&send_arp_data, iter->name);
        }
    }
    ...
    SMAP_FOR_EACH (node, &local_ecmp_nexthop_map) {
        ...
        if (pb) {
            send_arp_update(pb, node->key, max_arp_timeout,
                            continuous_arp);
        }
    }
    ...
    arp_max_timeout = max_arp_timeout;
    arp_continuous = continuous_arp;
}
