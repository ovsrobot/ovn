                          ...
                          struct dp_packet *pkt_in, const struct match *md)
{
    ...
    if (ipv6_string_mapped(ip6_s, &ip_flow->ipv6_src)) {
        VLOG_INFO_RL(&rl, "Received DHCPv6 advt from %s with aid %d"
    ...
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    if (status) {
        ...
        if (ipv6_string_mapped(ip6_s, &ip_flow->ipv6_src) &&
            ...
            VLOG_INFO_RL(&rl, "Received DHCPv6 reply from %s with prefix %s/%d"
    ...
    OVS_REQUIRES(pinctrl_mutex)
{
    ...
    for (size_t i = 0; i < ld->n_peer_ports; i++) {
        ...
        if (!pfd) {
            ...
            VLOG_INFO_RL(&rl, "updating port_binding for %s with prefix %s/%d"
