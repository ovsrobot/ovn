...
struct ovs_chassis_cfg {
    ...
    const char *evpn_local_ip_mapping;
...
static const char *
get_evpn_local_ip_mapping(const struct smap *ext_ids, const char *chassis_id)
{
    return get_chassis_external_id_value(ext_ids, chassis_id,
                                         "ovn-evpn-local-ip-mapping", "");
}
                         ...
                         struct ovs_chassis_cfg *ovs_cfg)
{
    ...
    ovs_cfg->evpn_local_ip_mapping =
        get_evpn_local_ip_mapping(&cfg->external_ids, chassis_id);
                           ...
                           struct smap *config)
{
    ...
    smap_replace(config, "ovn-evpn-local-ip-mapping",
                 ovs_cfg->evpn_local_ip_mapping);
                             ...
                             const struct sbrec_chassis *chassis_rec)
{
    ...
    const char *chassis_evpn_local_ip_mapping =
        get_evpn_local_ip_mapping(&chassis_rec->other_config,
                                  chassis_rec->name);
    if (strcmp(ovs_cfg->evpn_local_ip_mapping,
               chassis_evpn_local_ip_mapping)) {
        return true;
    }
...
update_supported_sset(struct sset *supported)
{
    ...
    sset_add(supported, "ovn-evpn-local-ip-mapping");
