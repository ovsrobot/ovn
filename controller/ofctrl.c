           ...
           bool pflows_changed)
{
    ...
    HMAP_FOR_EACH (m_desired, hmap_node, &meters->desired) {
        if (!ovn_extend_table_lookup(&meters->existing, m_desired)) {
            ...
            ofctrl_meter_bands_sync(m_desired, meter_table, &msgs);
        }
    }
