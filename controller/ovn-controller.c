...
#undef OVS_NODE
    ...
    /* Any changes to the runtime_data engine, need not be handled
     * for pflow_outout engine.  pflow_output_sb_port_binding_handler()
     * will generate the necessary physical flows.  We still
     * need runtime_data as input to access the runtime data in
     * physical.c and the hence noop handler. */
