...
{
    if (ts_lrp) {
        const char *lrp_opt = smap_get(&ts_lrp->options,
                                       "ic-route-ipv4-next-hop-ipv6");
        if (lrp_opt) {
            return !strcmp(lrp_opt, "true");
        }
    }
    return smap_get_bool(&nb_lr->options, "ic-route-ipv4-next-hop-ipv6",
                         false);
}
