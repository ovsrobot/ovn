...
az_run(struct ic_context *ctx)
{
    ...
    if (ctx->ovnisb_unlocked_txn && az_name && strcmp(az_name, nb_global->name)) {
...
main(int argc, char *argv[])
{
    ...
    struct ovsdb_idl_loop ovnisb_unlocked_idl_loop = OVSDB_IDL_LOOP_INITIALIZER(
        ...
        if (!state.paused) {
            ...
            if (new_ovnisb_unlocked_cond_seqno != ovnisb_unlocked_cond_seqno) {
                if (!new_ovnisb_unlocked_cond_seqno) {
                    VLOG_INFO("OVN ISB IDL Unlocked reconnected, force recompute.");
                    inc_proc_ic_force_recompute();
                }
            ...
            if (ovsdb_idl_has_lock(ctx.ovnsb_idl) &&
                ...
                if (!ovsdb_idl_loop_commit_and_wait(&ovnisb_unlocked_idl_loop)) {
