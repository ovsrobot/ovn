...
static void
sync_tsp_port(struct ic_context *ctx,
              ...
              const struct nbrec_logical_switch_port *lsp)
{
    ...
    } else if (strcmp(tsp->addresses[0], isb_pb->address)) {
    ...
    /* Sync router-id information to ISB. */
    struct uuid uuid;
    if (!strcmp(tsp->type, "router")) {
        if (tsp->peer[0]) {
            const struct nbrec_logical_router *lr = NULL;
            const struct nbrec_logical_router *iter = NULL;
            NBREC_LOGICAL_ROUTER_FOR_EACH (iter, ctx->ovnnb_idl) {
                for (size_t i = 0; i < iter->n_ports; i++) {
                    if (!strcmp(tsp->peer, iter->ports[i]->name)) {
                        lr = iter;
                        break;
                    }
                }
            ...
            if (lr) {
                if (!smap_get_uuid(&isb_pb->external_ids, "router-id", &uuid)
                    || !uuid_equals(&lr->header_.uuid, &uuid)) {
                    char *uuid_s =
                        xasprintf(UUID_FMT, UUID_ARGS(&lr->header_.uuid));
                    icsbrec_port_binding_update_external_ids_setkey(
                        isb_pb, "router-id", uuid_s);
                    free(uuid_s);
                }
            } else {
                if (smap_get_uuid(&isb_pb->external_ids, "router-id", &uuid)) {
                    icsbrec_port_binding_update_external_ids_delkey(
                        isb_pb, "router-id");
                }
            }
        }
        ...
        if (smap_get_uuid(&isb_pb->external_ids, "router-id", &uuid)) {
            icsbrec_port_binding_update_external_ids_delkey(isb_pb,
                                                            "router-id");
        }
    }
                ...
                const struct nbrec_logical_switch_port *lsp)
{
    ...
    if (!tsp->n_addresses) {
        ...
        if (strcmp(lsp->addresses[0], tsp->addresses[0])) {
            nbrec_logical_switch_port_set_addresses(
                lsp, (const char **) &tsp->addresses[0],
                tsp->n_addresses);
        }
    } else {
        nbrec_logical_switch_port_set_addresses(
            lsp, (const char **) &tsp->addresses[0], 1);
    }
    ...
    if (strcmp(tsp->type, "router")) {
        ...
        if (smap_get(&lsp->options, "router-port")) {
            nbrec_logical_switch_port_update_options_delkey(lsp,
                                                            "router-port");
        }
    } else {
...
port_binding_run(struct ic_context *ctx)
{
    ...
    ICNBREC_TRANSIT_SWITCH_FOR_EACH (ts, ctx->ovninb_idl) {
        ...
        for (size_t i = 0; i < ts->n_ports; i++) {
            ...
            if (is_owner) {
                sync_tsp_port(ctx, tsp, isb_pb, lsp);
            }
